...
export function Totales() {
  ...
  const calcularTotales = useCallback(() => {
    ...
    const subtotal = d.reduce((acc, item) => acc + (item.precioVenta * item.cantidad), 0);
    const descuento = d.reduce((acc, item) => acc + ((item.precioVenta - item.precio) * item.cantidad), 0);
