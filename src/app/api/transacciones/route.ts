...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [transacciones] = await conn.query("SELECT *, DATE_FORMAT(fecha, '%d/%m/%Y %r') AS f FROM transacciones WHERE DATE(fecha) = ?", [fecha]);
...
export async function POST(req: Request) {
  try {
    ...
    const transaccionValida = TransaccionSchema.parse({ ...data, fecha: format({ date: new Date(), format: "YYYY-MM-DD HH:mm:ss", tz: "America/Tegucigalpa" }) });
