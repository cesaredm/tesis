...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const factura of facturas) {
      const [detalles] = await conn.query<RowDataPacket[]>("SELECT d.id, d.factura facturaid, d.cantidad, d.precio, d.importe, d.precioVenta,p.descripcion, p.modelo, p.marca, p.id producto FROM detalles d join inventariotienda p on d.producto = p.id where d.factura = ?", [factura.id]);
