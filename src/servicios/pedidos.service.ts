...
class PedidosService {
  async getPedidos(): Promise<Pedido[]> {
    const { data } = await axios.get("/api/inventario/pedidos");
    return data;
  }

  async guardarPedido({ pedido, detalles }: { pedido: PedidoSave; detalles: DetallesPedidoSave[] }): Promise<RespuestaApi> {
    const { data } = await axios.post("/api/inventario/pedidos", { pedido, detalles });
    return data;
  }

  async actualizarPedido(pedido: PedidoUpdate): Promise<RespuestaApi> {
    const { data } = await axios.patch("/api/inventario/pedidos", pedido);
    return data;
  }
